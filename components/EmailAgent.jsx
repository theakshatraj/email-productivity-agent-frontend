  ...
  return (
    <div className="card p-0 overflow-hidden flex flex-col h-full">
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
        ...
        <button type="button" onClick={() => setContextOpen(true)} className="btn-secondary text-sm">Context</button>
      </div>

      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-6 pt-4 pb-2 border-b border-slate-100 flex-shrink-0">
          <div className="flex flex-wrap gap-2">
            {quick.map((q) => (
              <button key={q.label} type="button" onClick={() => send(q.payload)} className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs hover:bg-slate-50 transition">
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex-shrink-0 space-y-2">
          <div className="flex items-end gap-2">
            <textarea
              ...
              onKeyDown={onKeyDown}
              rows={2}
              className="flex-1 resize-none rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          ...
          <div className="flex items-center justify-between text-xs text-slate-500">
            <span>{input.length}/{limit} characters</span>
